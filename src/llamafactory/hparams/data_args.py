...
class DataArguments:
    ...
    # )
    max_image_length: int = field(
        default=None,
        metadata={"help": "The maximum image number in a chat message."},
    )
    exclude_id: List[str] = field(
        default=None,
        metadata={"help": "Exclude id."},
    )
    relabel_csv: str = field(
        default=None,
        metadata={"help": "Path the relabel csv. Should have `id`, `label` columns"},
    )
    image_dir: Optional[List[str]] = field(
        default=[""],
        metadata={"help": "Path to the folder containing the images or videos, or a list of such paths. Defaults to `dataset_dir`."},
    )
