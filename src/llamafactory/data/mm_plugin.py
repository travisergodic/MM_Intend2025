...
def random_blur(image, min_radius=2.0, max_radius=2.0):
...
def random_resize_and_pad(image, max_scale, min_scale):
    ...
    padded_image = Image.new("RGB", (original_width, original_height), (0, 0, 0))
    
    top_left_x = (original_width - new_width) // 2
    top_left_y = (original_height - new_height) // 2

    padded_image.paste(resized_image, (top_left_x, top_left_y))
    return padded_image

def random_jpeg_compression(img, max_quality, min_quality):
    quality = random.randint(min_quality, max_quality+1)
    buffer = io.BytesIO()
...
class BasePlugin:
    ...
    def _preprocess_image(self, image: "ImageObject", **kwargs) -> "ImageObject":
        ...
        """
        image_resolution: int = kwargs.get("image_resolution")
        random_resize_max_pixel = kwargs.get("random_resize_max_pixel")
        random_resize_min_pixel = kwargs.get("random_resize_min_pixel")
        max_pixel = kwargs.get("max_pixel")
        grayscale_prob = kwargs.get("grayscale_prob")
        blur_prob = kwargs.get("blur_prob")
        max_blur_radius = kwargs.get("max_blur_radius")
        brightness_prob = kwargs.get("brightness_prob")
        brightness_delta = kwargs.get("brightness_delta")
        do_jpeg_compression = kwargs.get("do_jpeg_compression")
        max_quality = kwargs.get("max_quality")
        min_quality = kwargs.get("min_quality")

        if random_resize_max_pixel and random_resize_min_pixel:
            max_pixel = random.uniform(random_resize_max_pixel, random_resize_min_pixel+1)

        if max_pixel is not None:
            if image.width * image.height > max_pixel:
                resize_factor = math.sqrt(max_pixel / (image.width * image.height))
                width, height = int(image.width * resize_factor), int(image.height * resize_factor)
                image = image.resize((width, height), resample=Image.NEAREST)
        else:
            if max(image.width, image.height) > image_resolution:
        ...
        if random.uniform(0, 1) < grayscale_prob:
            image = image.convert("L")

        if random.uniform(0, 1) < blur_prob:
            image = random_blur(image, min_radius=0.5, max_radius=max_blur_radius)

        if random.uniform(0, 1) < brightness_prob:
            image = random_brightness(image, 1-brightness_delta, 1+brightness_delta)

        if do_jpeg_compression:
            image = random_jpeg_compression(image, max_quality, min_quality)
    ...
    def _get_mm_inputs(
        ...
        if len(images) != 0:
            images = self._regularize_images(
                ...
                max_pixel=getattr(processor, "max_pixel", None),
                grayscale_prob=getattr(processor, "grayscale_prob", 0.0),
                blur_prob=getattr(processor, "blur_prob", 0.0),
                brightness_prob=getattr(processor, "brightness_prob", 0.0),
                max_blur_radius=getattr(processor, "max_blur_radius", 0.0),
                brightness_delta=getattr(processor, "brightness_delta", 0.0),
                do_jpeg_compression=getattr(processor, "do_jpeg_compression", False),
                max_quality=getattr(processor, "max_quality", 100),
                min_quality=getattr(processor, "min_quality", 50),
                random_resize_max_pixel=getattr(processor, "random_resize_max_pixel", None),
                random_resize_min_pixel=getattr(processor, "random_resize_min_pixel", None)
            )
